<nav>
    <div class="contact-data">
        <div class="contact-data__image-link">
            <a class="index-page-link" href="/index.php">
                <img class="index-page-logo" src="/img/logo.svg" alt="main-page-logo">
            </a>
        </div>
        <div class="contact-data-separator"></div>
        <div class="contact-data__contact-numbers">
            <p class="contact-numbers__contact-number">8 (34248) 45-914</p>
            <p class="contact-numbers__contact-number">8-909-100-10-30</p>
        </div>
        <div class="contact-data-separator"></div>
        <div class="contact-data__address">
            <p class="address__info">Пермский край, г. Губаха, пр-т Ленина, д. 45, оф. 4, пом. 1</p>
        </div>
    </div>
    <div class="navigation-buttons">
            <button class="navigation-buttons__index-button">Главная</button>
            <button class="navigation-buttons__about-button">О компании</button>
            <button class="navigation-buttons__personal-area">Личный кабинет</button>
    </div>
</nav>
